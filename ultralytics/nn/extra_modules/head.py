...
class Detect_DyHead(nn.Module):
    ...
    def __init__(self, nc=3, hidc=256, block_num=2, ch=()):  # detection layer
        super().__init__()
...
class Detect_DyHeadWithDCNV3(Detect_DyHead):
    def __init__(self, nc=3, hidc=256, block_num=2, ch=()):
...
class Detect_Efficient(nn.Module):
    ...
    def __init__(self, nc=3, ch=()):  # detection layer
        ...
        # self.stem = nn.ModuleList(nn.Sequential(ScConv(x), Conv(x, x, 1)) for x in ch) # one 1x1 ScConv(CVPR2023), one 1x1 Conv  15.4
        # self.stem = nn.ModuleList(nn.Sequential(SCConv(x, x), Conv(x, x, 1)) for x in ch) # one 1x1 ScConv(CVPR2020), one 1x1 Conv
        # self.stem = nn.ModuleList(nn.Sequential(DiverseBranchBlock(x, x, 3), DiverseBranchBlock(x, x, 3)) for x in ch) # two 3x3 DiverseBranchBlock 39
        # self.stem = nn.ModuleList(nn.Sequential(RepConv(x, x, 3), RepConv(x, x, 3)) for x in ch) # two 3x3 RepConv   26.3
        # self.stem = nn.ModuleList(nn.Sequential(Partial_conv3(x, 4), Conv(x, x, 1)) for x in ch) # one PConv(CVPR2023), one 1x1 Conv  14.6
