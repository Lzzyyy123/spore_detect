...
def non_max_suppression(
        ...
        labels=(),
        max_det=300,
        nc=3,  # number of classes (optional)
        max_time_img=0.05,
        max_nms=30000,
        max_wh=7680,
):
...
def box_iou_for_nms(box1, box2, GIoU=False, DIoU=True, CIoU=False, SIoU=False, EIou=False, eps=1e-7):
...
def NMS(boxes, scores, iou_thres, GIoU=True, DIoU=False, CIoU=False):
    ...
    while B.numel() > 0:
        ...
        keep.append(index)
        if B.numel() == 1: break
        iou = box_iou_for_nms(boxes[index, :], boxes[B[1:], :], GIoU=GIoU, DIoU=DIoU, CIoU=CIoU).squeeze()
