...
class WIoU_Scale:
    ...
    iou_mean = 1.
    monotonous = False
    _momentum = 1 - 0.05 ** (1 / 3200)              #  number of batches is n  batches = 16 t =200 t*n = 3200
    _is_train = True
...
def bbox_mpdiou(box1, box2, xywh=True, mpdiou_hw=1, eps=1e-7,gamma=0.5, Focal=False):
    """
    Calculate Intersection over Union (IoU) of box1(1, 4) to box2(n, 4).
    """
    ...
    if Focal:
        return iou - d1 / mpdiou_hw.unsqueeze(1) - d2 / mpdiou_hw.unsqueeze(1) , torch.pow(inter / (union + eps), gamma)  # MPDIoU
    else:
        return iou - d1 / mpdiou_hw.unsqueeze(1) - d2 / mpdiou_hw.unsqueeze(1)
...
class Metric(SimpleClass):
    ...
    def fitness(self):
        ...
        w = [0.2, 0.2, 0.6, 0.0]  # weights for [P, R, mAP@0.5, mAP@0.5:0.95]
