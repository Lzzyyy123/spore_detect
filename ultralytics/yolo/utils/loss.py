...
from .metrics import bbox_iou, bbox_mpdiou, bbox_inner_mpdiou, wasserstein_loss
...
class EMASlideLoss:
    ...
    def __call__(self, pred, true, auto_iou=0.5):
        if self.is_train and auto_iou != -1:
            ...
            # self.iou_mean = (d * self.iou_mean + (1 - d) * float(auto_iou.detach())) / (1-d**self.updates) #  偏差修正ema
...
class TrapezoidLoss(nn.Module):
    def __init__(self, loss_fcn):
        super(TrapezoidLoss, self).__init__()
        self.loss_fcn = loss_fcn
        self.reduction = loss_fcn.reduction
        self.loss_fcn.reduction = 'none'  # required to apply SL to each element
    ...
    def forward(self, pred, true, auto_iou):
        ...
        if auto_iou < 0.25:
            auto_iou = 0.25
        b1 = true <= auto_iou - s
        a1 = (math.exp(1.0 - auto_iou) - 1.0) / (auto_iou - s) * true + 1.0

        b2 = (true > (auto_iou - s)) & (true < auto_iou)
        a2 = math.exp(1.0 - auto_iou)

        b3 = true >= auto_iou
        a3 = ((math.exp(1.0 - auto_iou) - 1.0) / (auto_iou-1.0)) * (true-1) + 1.0
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask, mpdiou_hw=None):
        ...
        iou = bbox_mpdiou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, mpdiou_hw=mpdiou_hw[fg_mask],Focal=True)

        if type(iou) is tuple:
            loss_iou = ((1.0 - iou[0]) * iou[1].detach() * weight).sum() / target_scores_sum       #iou[1].detach() IOU`r
        else:
...
class v8DetectionLoss:
    def __init__(self, model):  # model must be de-paralleled
        ...
        # self.bce = VFLoss(nn.BCEWithLogitsLoss(reduction='none')) # VFLoss
        # self.bce = TrapezoidLoss(nn.BCEWithLogitsLoss(reduction='none')) # TrapezoidLoss
        self.hyp = h
        self.stride = m.stride  # model scxztrides
    ...
    def compute_loss(self, preds, batch):
        ...
        if isinstance(self.bce, (EMASlideLoss, SlideLoss, TrapezoidLoss)):
