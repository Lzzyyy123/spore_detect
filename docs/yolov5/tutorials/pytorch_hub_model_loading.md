...
```python
model = torch.hub.load('ultralytics/yolov5', 'custom', path='path/to/yolov8s.pt')  # local model
model = torch.hub.load('path/to/yolov5', 'custom', path='path/to/yolov8s.pt', source='local')  # local repo
```
