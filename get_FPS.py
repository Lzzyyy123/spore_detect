...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default=r'YOLOv8s-ACFFPN.pt', help='trained weights path ')
    ...
    parser.add_argument('--warmup', default=0, type=int, help='warmup time')
    ...
    if weights.endswith('.pt'):
        print('YOLOv8s inference:')
        print('-------------------------------------------------------------------------------------------')
        print(f'model weights:{opt.weights} size:{get_weight_size(opt.weights)}M (bs:{opt.batch})Latency:{infer_time_per_image:.5f}s +- {std_time:.5f}s fps:{1 / (infer_time_per_image):.2f}')
        print('-------------------------------------------------------------------------------------------')   
    else:
        print(f'model yaml:{opt.weights} (bs:{opt.batch})Latency:{infer_time_per_image:.5f}s +- {std_time:.5f}s fps:{1 / infer_time_per_image:.2f}')